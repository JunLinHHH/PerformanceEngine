...
% Load the figure
fig = openfig('I:\CH4DDI\Mark‘s paper\JournalFigures\Figure_HRR_DualFuel_Main_Pilot.fig');

% Get all axes
allAxes = findall(fig, 'Type', 'axes');
fprintf('========== TOTAL AXES FOUND: %d ==========\n', length(allAxes));

% Sort axes by position (top to bottom)
...
[~, sortIdx] = sort(axesPositions(:, 2), 'descend');
...
if length(plotAxes) >= 2
    topSubplot = plotAxes(1);    % Top subplot (with shaded areas)
    bottomSubplot = plotAxes(2); % Bottom subplot (clean average lines)
else
    error('Expected at least 2 subplots, found %d', length(plotAxes));
end

fprintf('\n========== ANALYZING TOP SUBPLOT (with variations) ==========\n');
topChildren = get(topSubplot, 'Children');
fprintf('Top subplot children: %d\n', length(topChildren));

% Separate patch objects (shaded areas) and line objects
topPatches = findall(topSubplot, 'Type', 'patch');
topLines = findall(topSubplot, 'Type', 'line');

fprintf('Top subplot patches (shaded areas): %d\n', length(topPatches));
fprintf('Top subplot lines: %d\n', length(topLines));

fprintf('\n========== ANALYZING BOTTOM SUBPLOT (average lines) ==========\n');
bottomChildren = get(bottomSubplot, 'Children');
fprintf('Bottom subplot children: %d\n', length(bottomChildren));

bottomLines = findall(bottomSubplot, 'Type', 'line');
fprintf('Bottom subplot lines: %d\n', length(bottomLines));

% Extract average lines from bottom subplot
fprintf('\n========== EXTRACTING AVERAGE LINES (Bottom Subplot) ==========\n');
averageLineData = struct();

for i = 1:length(bottomLines)
    currentLine = bottomLines(i);
...
% Extract shaded areas from top subplot
fprintf('\n========== EXTRACTING SHADED AREAS (Top Subplot) ==========\n');
shadedAreaData = struct();

for i = 1:length(topPatches)
    currentPatch = topPatches(i);
...
% Also extract mean lines from top subplot for comparison
fprintf('\n========== EXTRACTING MEAN LINES (Top Subplot) ==========\n');
topMeanLineData = struct();
...
for i = 1:length(topLines)
    ...
    % Store in structure
    topMeanLineData(i).xData = xData;
    topMeanLineData(i).yData = yData;
    topMeanLineData(i).color = color;
    topMeanLineData(i).displayName = displayName;
...
fprintf('Top mean lines extracted: %d\n', length(topMeanLineData));
...
% Process each average line (bottom subplot data)
