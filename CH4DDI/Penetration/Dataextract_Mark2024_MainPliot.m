...
% First, detect the actual colors from solid lines
fprintf('\n========== DETECTING COLORS FROM SOLID LINES ==========\n');
for i = 1:length(allAxes)
    lines = findall(allAxes(i), 'Type', 'line');
    solidCount = 0;
    for j = 1:length(lines)
        if strcmp(lines(j).LineStyle, '-') && length(lines(j).XData) > 10
            solidCount = solidCount + 1;
            fprintf('Axes %d, Solid line %d: Color=[%.3f %.3f %.3f], %d points\n', ...
                i, solidCount, lines(j).Color(1), lines(j).Color(2), ...
                lines(j).Color(3), length(lines(j).XData));
        end
    end
end

fprintf('\n========== IDENTIFYING D-CASE AXES ==========\n');
fprintf('Looking for the 4 subplots with data...\n');
...
for i = 1:length(allAxes)
    ...
    if yPos < 100 && numLinesWithData >= 3
        ...
        fprintf('Found subplot at axes %d: Y=%.1f, Lines with data=%d\n', ...
            i, yPos, numLinesWithData);
    end
end

fprintf('\nSubplot axes indices: %s\n', mat2str(dCaseAxes));

% Define subplot names - UPDATED FOR NEW FIGURE
subplotNames = {'D_0_03ms_M', 'M_0_97ms_D', 'M_1_97ms_D', 'M_2_97ms_D'};
subplotTitles = {'D-0.03ms-M', 'M-0.97ms-D', 'M-1.97ms-D', 'M-2.97ms-D'};
...
for dataIdx = 1:min(4, length(dCaseAxes))
    ...
    fprintf('\n--- Processing subplot %d: Axes index %d (%s) ---\n', ...
    ...
    for i = 1:length(lines)
        ...
        allData{dataIdx}.(fieldName).DisplayName = displayName;
...
if strcmp(answer, 'Yes - Save the data')
    ...
    % Save with correct variable names for this figure
    ...
    M_0_97ms_D = allData{2};
    save(fullfile(saveFolder, 'Mark2024_Figure_extractedData_M_0_97ms_D.mat'), 'M_0_97ms_D');
    fprintf('  ✓ Saved: M_0_97ms_D\n');
    
    M_1_97ms_D = allData{3};
    save(fullfile(saveFolder, 'Mark2024_Figure_extractedData_M_1_97ms_D.mat'), 'M_1_97ms_D');
    fprintf('  ✓ Saved: M_1_97ms_D\n');
    
    M_2_97ms_D = allData{4};
    save(fullfile(saveFolder, 'Mark2024_Figure_extractedData_M_2_97ms_D.mat'), 'M_2_97ms_D');
    fprintf('  ✓ Saved: M_2_97ms_D\n');
