...
ch4_files = {
    ...
    'C:\Users\jun-y\OneDrive - UNSW\CH4DDI\CH4DDI vs H2DDI CVCC\Mark2024_handled\Mark2024_Figure_extractedData_M_0_97ms_D.mat';
    'C:\Users\jun-y\OneDrive - UNSW\CH4DDI\CH4DDI vs H2DDI CVCC\Mark2024_handled\Mark2024_Figure_extractedData_M_1_97ms_D.mat';
    'C:\Users\jun-y\OneDrive - UNSW\CH4DDI\CH4DDI vs H2DDI CVCC\Mark2024_handled\Mark2024_Figure_extractedData_M_2_97ms_D.mat'
};
...
h2_files = {
    ...
    'C:\Users\jun-y\OneDrive - UNSW\CH4DDI\CH4DDI vs H2DDI CVCC\Patrick2022_handled\H-1.07ms-D_Patrick2022_Figure13_extracted_data.mat';
    'C:\Users\jun-y\OneDrive - UNSW\CH4DDI\CH4DDI vs H2DDI CVCC\Patrick2022_handled\H-2.07ms-D_Patrick2022_Figure13_extracted_data.mat';
};
...
% CH4 shift values
ch4_shift = [0.69, 1.89, 2.91, 3.95];

% Plot CH4 velocities for all cases (with shifting)
for i = 1:4
    case_name = ['case_' num2str(i)];
    if isfield(CH4_data, case_name) && ~isempty(fieldnames(CH4_data.(case_name)))
        current_ch4 = CH4_data.(case_name);
        
        % Flame penetration velocity - with shifting
        if ~isempty(current_ch4.x_penetration) && ~isempty(current_ch4.y_penetration)
            velocity_ch4 = gradient(current_ch4.y_penetration) ./ gradient(current_ch4.x_penetration);
            velocity_ch4 = movmean(velocity_ch4, smooth_window_ch4, 'omitnan');
            
            % Apply shift: subtract the shift value
            x_ch4_shifted = current_ch4.x_penetration - ch4_shift(i);
            
            plot(x_ch4_shifted, velocity_ch4, line_styles{1}, ...
...
% H2 shift values (same as cut points)
h2_shift = [0.83, 1.70, 2.69, 3.49];
%h2_shift = [0,0,0,0];
% Plot H2 velocities for all cases (with shifting)
for i = 1:4
    case_name = ['case_' num2str(i)];
    if isfield(H2_data, case_name) && ~isempty(fieldnames(H2_data.(case_name)))
        current_h2 = H2_data.(case_name);
        
        % Flame penetration average velocity (green equivalent) - with shifting
        if ~isempty(current_h2.x_green) && ~isempty(current_h2.y_green)
            velocity_h2 = gradient(current_h2.y_green) ./ gradient(current_h2.x_green);
            velocity_h2 = movmean(velocity_h2, smooth_window_h2, 'omitnan');
            
            % Apply shift: subtract the shift value
            x_h2_shifted = current_h2.x_green - h2_shift(i);
            
            plot(x_h2_shifted, velocity_h2, line_styles{1}, ...
                'Color', h2_colors(i,:), 'LineWidth', 3, ...
                'DisplayName', [current_h2.case_name]);
        end
        
%         %Flame penetration velocity (blue equivalent) - with shifting
%         if ~isempty(current_h2.x_blue) && ~isempty(current_h2.y_blue) && ~isscalar(current_h2.x_blue)
%             velocity_h2_blue = gradient(current_h2.y_blue) ./ gradient(current_h2.x_blue);
%             velocity_h2_blue = movmean(velocity_h2_blue, smooth_window_h2, 'omitnan');
%             
%             Apply shift: subtract the shift value
%             x_h2_blue_shifted = current_h2.x_blue - h2_shift(i);
%             
%             plot(x_h2_blue_shifted, velocity_h2_blue, line_styles{1}, ...
%                 'Color', h2_colors(i,:), 'LineWidth', 3, ...
%                 'DisplayName', [current_h2.case_name]);
%         end
    end
...
ch4_shift = [0.69, 1.89, 2.91, 3.95]; % Cut points for each H2 case
...
% Plot H2 velocities for all cases (shifted to zero)
 h2_cut_points = [0.69, 1.70, 2.69, 3.49]; % Cut points for each H2 case
...
for i = 2:4
    ...
    if isfield(H2_data, case_name) && ~isempty(fieldnames(H2_data.(case_name)))
        ...
        if ~isempty(current_h2.x_green) && ~isempty(current_h2.y_green)
            ...
            plot(x_h2_green_shifted, velocity_h2_cut, line_styles{1}, ...
                'Color', h2_colors(i,:), 'LineWidth', 3, ...
                'DisplayName', [current_h2.case_name]);
        end
